...
@Component({
  selector: 'app-pasts',
  templateUrl: './pasts.component.html',
  styleUrls: ['./pasts.component.scss'],
})
export class PastsComponent implements OnInit {
  loading = false;
  items: Launch[] = [];
  ...
  fetchData(page: number, pageSize: number, order: string, sort: string) {
    ...
    const params = {};
    this.service
      .launchesPastQuery(params, limit, offset, order, sort)
      .subscribe(
        (res) => {
          this.loading = false;
          this.items = res;
          ...
          this.location.replaceState('/timelines/pasts', query);
        },
        () => {
          this.loading = false;
        }
